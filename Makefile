...
ISDB_SCANNER_VERSION := $(shell cat .isdb-scanner-version | tr -d '[:space:]')

# Get the latest tag matching <isdb-scanner-version>-<number> pattern
LATEST_TAG     := $(shell git tag --sort=-v:refname | grep -E "^$(ISDB_SCANNER_VERSION)-[0-9]+$$" | head -n1)
LATEST_BUILD   := $(shell echo "$(LATEST_TAG)" | sed 's/.*-//')
NEXT_BUILD     := $(shell if [ -n "$(LATEST_BUILD)" ]; then expr $(LATEST_BUILD) + 1; else echo 1; fi)
NEXT_TAG       := $(ISDB_SCANNER_VERSION)-$(NEXT_BUILD)
...
release: ## Release a new build. Usage: make release [dryrun=false]
	@echo "isdb-scanner version : $(ISDB_SCANNER_VERSION)"
	...
	@if [ "$(dryrun)" = "false" ]; then \
		...
		echo "[DRY RUN] Would push origin/master"; \
